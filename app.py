...
st.set_page_config(layout="wide", page_title="📱 Phân tích & Dự đoán Giá Điện Thoại 2020–2027")

# Load data
@st.cache_data
def load_data():
    return pd.read_csv("phones_vn_extended_2022_2027.csv", encoding="utf-8-sig")
...
brands = st.sidebar.multiselect(
    "Chọn hãng",
    options=sorted(df['Hãng'].unique()),
    default=sorted(df['Hãng'].unique())
)

# Năm ra mắt từ 2020–2027
...
os_choices = st.sidebar.multiselect(
    "Hệ điều hành",
    options=sorted(df['Hệ điều hành'].unique()),
    default=sorted(df['Hệ điều hành'].unique())
)
...
model_choice = st.selectbox("Chọn model", ["Hồi quy tuyến tính", "Rừng ngẫu nhiên"])
...
c1.metric("Sai số MAE", f"{mean_absolute_error(y_test, y_pred):,.0f}")
c2.metric("Sai số RMSE", f"{np.sqrt(mean_squared_error(y_test, y_pred)):,.0f}")
...
with st.form("predict_form"):
    ...
    year = st.number_input("Năm ra mắt", min_value=2020, max_value=2027, value=2023)
